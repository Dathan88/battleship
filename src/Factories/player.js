...
const Player = (name = 'Player 1') => {
	...
	const random_attack = enemy => {
		...
		const shotCondition = board.shotsFired.find(function(el) {
			return el === x;
		});
		const shot = board.incoming(x);

		if (outcome.test(shot) === true && shotCondition === undefined) {
			shots_hit.push(x);
			return enemy.getName() + "'s " + shot;
		} else if (outcome.test(shot) === false && shotCondition === undefined) {
			shots_missed.push(x);
			return name + ' missed with ' + x + ' and it is ' + player_turn();
		} else {
			turn--;
